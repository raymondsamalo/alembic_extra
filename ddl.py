...
class DDLHandler(ABC):
    ...
    def ddl_name_sql_query_from_schema(self, schema_name: str) -> str:
        ...
        e.g.
            find table_name or function_name and transform it to match our unique ddl_name
        :param schema_name:
...
def compare_ddl(autogen_context, upgrade_ops, schemas):
    ...
    for sch in schemas:
        ...
        for ddl_name, ddl_handler in ddl_registry.items():
            statement = ddl_handler.ddl_name_sql_query_from_schema(schema_name)
            result = autogen_context.connection.execute(text(statement))
            metadata_in_database.update([(ddl_name, sch, row[0]) for row in result])
